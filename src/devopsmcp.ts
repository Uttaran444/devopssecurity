...
export async function makeApiCall(
  ...
  try {
    ...
    // Expect inbound Authorization: Bearer <user_token> from Copilot
    const userAuthHeader = (globalThis as any)?.transportContext?.headers?.authorization as string | undefined;
    let incomingToken = '';
  ...
  server.tool(
    ...
    async (args, context: RequestHandlerExtra<any, any>) => {
      ...
      try {
        ...
        const itemsResult = await makeApiCall('GET', itemsUrl, null, async (n:any) => { await safeNotification(context, n); });
        ...
        if (wantContent) {
          ...
          for (const m of matches) {
            ...
            try {
              ...
              const fileResult = await makeApiCall('GET', fileUrl, null, async (n:any) => { await safeNotification(context, n); });
